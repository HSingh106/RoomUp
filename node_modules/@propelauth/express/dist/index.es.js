...
import { constants, createPublicKey, createSecretKey, KeyObject } from 'node:crypto';
...
// src/api/user.ts
var ENDPOINT_PATH = "/api/backend/v1/user";
...
// src/api/org.ts
var ENDPOINT_PATH2 = "/api/backend/v1/org";
function fetchOrg(authUrl, integrationApiKey, orgId) {
  if (!isValidId(orgId)) {
    return Promise.resolve(null);
  }
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/${orgId}`, "GET").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
      throw new Error("integrationApiKey is incorrect");
    } else if (httpResponse.statusCode === 404) {
      return null;
...
function fetchOrgByQuery(authUrl, integrationApiKey, query) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/query`, "POST", JSON.stringify(request)).then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
function addUserToOrg(authUrl, integrationApiKey, addUserToOrgRequest) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/add_user`, "POST", JSON.stringify(request)).then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
function allowOrgToSetupSamlConnection(authUrl, integrationApiKey, orgId) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/${orgId}/allow_saml`, "POST").then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 404) {
...
function disallowOrgToSetupSamlConnection(authUrl, integrationApiKey, orgId) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/${orgId}/disallow_saml`, "POST").then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 404) {
...
function updateOrg(authUrl, integrationApiKey, updateOrgRequest) {
  ...
  return httpRequest(
    authUrl,
    integrationApiKey,
    `${ENDPOINT_PATH2}/${updateOrgRequest.orgId}`,
    "PUT",
    JSON.stringify(request)
  ).then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
...
function subscribeOrgToRoleMapping(authUrl, integrationApiKey, orgId, customRoleMappingName) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/${orgId}`, "PUT", JSON.stringify(request)).then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
function deleteOrg(authUrl, integrationApiKey, orgId) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH2}/${orgId}`, "DELETE").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
      throw new Error("integrationApiKey is incorrect");
    } else if (httpResponse.statusCode === 404) {
      return false;
...
var ENDPOINT_PATH4 = "/api/backend/v1/access_token";
...
var ENDPOINT_PATH5 = "/api/backend/v1/migrate_user";
function migrateUserFromExternalSource(authUrl, integrationApiKey, migrateUserFromExternalSourceRequest) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH5}/`, "POST", JSON.stringify(request)).then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
var ENDPOINT_PATH6 = "/api/backend/v1/end_user_api_keys";
function fetchApiKey(authUrl, integrationApiKey, apiKeyId) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH6}/${apiKeyId}`, "GET").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
      throw new Error("integrationApiKey is incorrect");
    } else if (httpResponse.statusCode === 400) {
...
function fetchCurrentApiKeys(authUrl, integrationApiKey, apiKeyQuery) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH6}?${queryString}`, "GET").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
      throw new Error("integrationApiKey is incorrect");
    } else if (httpResponse.statusCode === 400) {
      throw new ApiKeyFetchException(httpResponse.response);
...
function fetchArchivedApiKeys(authUrl, integrationApiKey, apiKeyQuery) {
  ...
  const queryString = formatQueryParameters(request);
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH6}/archived?${queryString}`, "GET").then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
function validateApiKey(authUrl, integrationApiKey, apiKeyToken) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH6}/validate`, "POST", JSON.stringify(request)).then(
    (httpResponse) => {
      if (httpResponse.statusCode === 401) {
        throw new Error("integrationApiKey is incorrect");
      } else if (httpResponse.statusCode === 400) {
...
function deleteApiKey(authUrl, integrationApiKey, apiKeyId) {
  ...
  return httpRequest(authUrl, integrationApiKey, `${ENDPOINT_PATH6}/${apiKeyId}`, "DELETE").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
      throw new Error("integrationApiKey is incorrect");
    } else if (httpResponse.statusCode === 400) {
...
var ENDPOINT_PATH7 = "/api/v1/token_verification_metadata";
function fetchTokenVerificationMetadata(authUrl, integrationApiKey, manualTokenVerificationMetadata) {
  ...
  return httpRequest(authUrl, integrationApiKey, ENDPOINT_PATH7, "GET").then((httpResponse) => {
    if (httpResponse.statusCode === 401) {
...
const getTokenVerificationMetadataPromise = (tokenVerificationMetadataPromise) => __awaiter$1(void 0, void 0, void 0, function* () {
...
function initAuth(opts) {
    ...
    const requireOrgMember = createRequireOrgMemberMiddleware(auth.validateAccessTokenAndGetUserWithOrgInfo, auth.validateAccessTokenAndGetUserClass, debugMode);
    const requireOrgMemberWithMinimumRole = createRequireOrgMemberMiddlewareWithMinimumRole(auth.validateAccessTokenAndGetUserWithOrgInfoWithMinimumRole, auth.validateAccessTokenAndGetUserClass, debugMode);
    const requireOrgMemberWithExactRole = createRequireOrgMemberMiddlewareWithExactRole(auth.validateAccessTokenAndGetUserWithOrgInfoWithExactRole, auth.validateAccessTokenAndGetUserClass, debugMode);
    const requireOrgMemberWithPermission = createRequireOrgMemberMiddlewareWithPermission(auth.validateAccessTokenAndGetUserWithOrgInfoWithPermission, auth.validateAccessTokenAndGetUserClass, debugMode);
    const requireOrgMemberWithAllPermissions = createRequireOrgMemberMiddlewareWithAllPermissions(auth.validateAccessTokenAndGetUserWithOrgInfoWithAllPermissions, auth.validateAccessTokenAndGetUserClass, debugMode);
    return {
        requireUser,
        optionalUser,
        requireOrgMember,
...
function createUserExtractingMiddleware({ validateAccessTokenAndGetUser, validateAccessTokenAndGetUserClass, requireCredentials, debugMode, }) {
    return function (req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                req.user = yield validateAccessTokenAndGetUser(req.headers.authorization);
                req.userClass = yield validateAccessTokenAndGetUserClass(req.headers.authorization);
                next();
            }
            catch (e) {
...
function createRequireOrgMemberMiddleware(validateAccessTokenAndGetUserWithOrgInfo, validateAccessTokenAndGetUserClass, debugMode) {
...
function createRequireOrgMemberMiddlewareWithMinimumRole(validateAccessTokenAndGetUserWithOrgInfoWithMinimumRole, validateAccessTokenAndGetUserClass, debugMode) {
    return function requireOrgMemberWithMinimumRole(args) {
        const orgIdExtractor = args ? args.orgIdExtractor : undefined;
        const orgNameExtractor = args ? args.orgNameExtractor : undefined;
        return requireOrgMemberGenericMiddleware((authorizationHeader, requiredOrgInfo) => {
...
function createRequireOrgMemberMiddlewareWithExactRole(validateAccessTokenAndGetUserWithOrgInfoWithExactRole, validateAccessTokenAndGetUserClass, debugMode) {
    return function requireOrgMemberWithMinimumRole(args) {
        const orgIdExtractor = args ? args.orgIdExtractor : undefined;
        const orgNameExtractor = args ? args.orgNameExtractor : undefined;
        return requireOrgMemberGenericMiddleware((authorizationHeader, requiredOrgInfo) => {
...
function createRequireOrgMemberMiddlewareWithPermission(validateAccessTokenAndGetUserWithOrgInfoWithPermission, validateAccessTokenAndGetUserClass, debugMode) {
    return function requireOrgMemberWithMinimumRole(args) {
        const orgIdExtractor = args ? args.orgIdExtractor : undefined;
        const orgNameExtractor = args ? args.orgNameExtractor : undefined;
        return requireOrgMemberGenericMiddleware((authorizationHeader, requiredOrgInfo) => {
...
function createRequireOrgMemberMiddlewareWithAllPermissions(validateAccessTokenAndGetUserWithOrgInfoWithAllPermissions, validateAccessTokenAndGetUserClass, debugMode) {
    return function requireOrgMemberWithMinimumRole(args) {
        const orgIdExtractor = args ? args.orgIdExtractor : undefined;
        const orgNameExtractor = args ? args.orgNameExtractor : undefined;
        return requireOrgMemberGenericMiddleware((authorizationHeader, requiredOrgInfo) => {
...
function requireOrgMemberGenericMiddleware(validateAccessTokenAndGetUserWithOrgInfo, validateAccessTokenAndGetUserClass, debugMode, orgIdExtractor, orgNameExtractor) {
    return function (req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            let requiredOrgInfo;
            ...
            try {
                const userAndOrgMemberInfo = yield validateAccessTokenAndGetUserWithOrgInfo(req.headers.authorization, requiredOrgInfo);
...
function handleUnauthorizedException({ exception, requireCredentials, res, next, debugMode, }) {
...
function handleForbiddenExceptionWithRequiredCredentials(exception, res, debugMode) {
    if (debugMode) {
        res.status(exception.status).send(exception.message);
    }
    else {
...
function handleUnexpectedException(exception, res, debugMode) {
    if (debugMode) {
        res.status(exception.status).send(exception.message);
    }
    else {
